...
export function PublicPollList() {
  ...
  const fetchPolls = useCallback(async () => {
    ...
    try {
      const pollsData = await getPublicPolls()
  ...
  if (displayPolls.length === 0) {
    return (
      <div className="text-center py-12 bg-gradient-to-r from-violet-50 to-pink-50 rounded-xl">
        <p className="text-muted-foreground mb-6 text-lg">No public polls available. Create one to get started!</p>
        <Button
          asChild
                ...
                <div className="flex justify-between items-start">
                  ...
                  {!poll.imageUrl && (
                    <Badge
                      ...
                      className={isPollActive ? "bg-gradient-to-r from-violet-600 to-pink-600 text-white border-0" : ""}
                    >
